...
class PostsRecyclerViewAdapter(val onItemClick: OnItemClick) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(
        ...
        val postViewHolder = (holder as PostsViewHolder)
        postViewHolder.binding(postsList.get(position))
        postViewHolder.setupListeners(postsList.get(position))
    }
    ...
    inner class PostsViewHolder(val bind: RecyclerItemBinding) :
        RecyclerView.ViewHolder(bind.root) {
        fun binding(post: Post) {
            bind.textTitle.text = post.title
        }

        fun setupListeners(post: Post) {
